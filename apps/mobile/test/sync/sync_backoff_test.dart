...
void main() {
  test(
    'delay grows exponentially with the attempt number, capped by maxDelay',
    () {
      ...
      expect(
        backoff.delayFor(4),
        Duration(seconds: 10),
      ); // base * 2^4 = 16s, capped at 10s
    },
  );

  test(
    'delay is never negative and never exceeds the uncapped exponential value',
    () {
      const backoff = SyncBackoff(
        baseDelay: Duration(seconds: 1),
